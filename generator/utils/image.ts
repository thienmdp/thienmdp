...
export async function generateImagePieces({ imageFilepath }: { imageFilepath: string }) {
  ...
  for (const row of imageConfig.rows) {
    ...
    for (const link of row.links) {
      ...
      const href = unparsedHref.replace("${LATEST_CONTENT_URL}", "https://www.tiktok.com/@thien-mdp/video/");
  ...
  const imagePieces: ImagePiece[] = await Promise.all(
    crops.map(async (crop) => {
      ...
      const buffer = await image.clone().extract(dimensions).toFormat("png").toBuffer();
      const bufferHash = await hash(buffer);
      const filepath = path.join(monorepoDirpath, "generated", `${bufferHash}.png`);
